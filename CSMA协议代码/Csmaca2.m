function [AllFrame SuccessFrame] = Csmaca2(NumberNodes,ConWindow)                          
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 步骤一 : 初始化
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if CurBufferSize(i) < BufferSize - FrameLength(i)              %如果帧缓冲区还可以未满
                ...
                if HasFrameFlag(i) == FALSE                                %当缓冲区没有帧，此时有帧进入
                    ...
                    if ChannelBusyFlag== FALSE                             %当信道空闲
                        ...
                        BackoffTime(i)=SetBackoffTime2(CountBackoff(1));                  %否则退避时间置为一随机时间
                    end
                end        
            end
            sign=sign+1;                                                   %不断生成新的帧
          ...
          for i = 1:NumberNodes
              if t == SendEndTimeTemp(i)
                ...
                if k == 0                                                  %如果缓冲区无帧
                    ...
                    BackoffTime(j) =SetBackoffTime2(j);                     %否则，还有帧，更新碰撞时间
                end    
              end 
          end
      end
